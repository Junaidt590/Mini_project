...
if(isset($_POST["submit"])){
    ...
    $ordeer_details_sql="INSERT INTO `order_details`(`ord_date`)VALUES('$Date')";
    $resultod = $con->query($ordeer_details_sql);
    $order_details_id = $con->insert_id;
    // end of odid
    ...
    $Date = date('Y-m-d');
        ...
        $stmt = $pdo->prepare("INSERT INTO ordertab (item_id,qty,price,status,uid,Proof,Date,order_detail_id) VALUES (?,?,?,?,?,?,?,?)");
        
        if($stmt->execute([ $item_id,$qty,$price,$status,$uid,prav_upload($Proof),$Date,$order_details_id  ])) {
                $stmt = null;
				
				$stmt = $pdo->prepare("delete from cart where uid=? and item_id=?");
				if($stmt->execute([$uid, $instrument_id])) {
                $stmt = null;
                //$stmt = $pdo->prepare("INSERT INTO order_details (ordno) VALUES (?)");
                //$sql="SELECT ordno FROM ordertab WHERE uid=$uid";
...
if(isset($_GET['id'])){
        ...
        $vars = parse_columns('ordertab', $_POST);
        $stmt = $pdo->prepare("INSERT INTO ordertab (item_id,qty,price,status,uid,Proof,Date) VALUES (?,?,?,?,?,?,?)");
...
{
    $uid=$_GET['uid'];
    $sql="select a.*,b.brand,b.item_name,b.image from cart as a,item as b where a.item_id=b.item_id and a.uid=$uid";
	  	$result = $con->query($sql);
  ...
  while($row = $result->fetch_assoc()) {
    ...
    //$sql="select sum(price) as price, (qty) as qty from cart where uid=$uid";
    //$sum=$con->query($sql);
    //$t=mysqli_fetch_assoc($sum);
    //$tsum=$t['price']*$t['qty'];
    //echo '<h1>'+$tsum+'</h1>';
    //echo $t['price'];
    $sql="select * from item where item_id in (select item_id from cart where uid=$uid)";
    $in=$con->query($sql);
...
<body>
    ...
    <section class="pt-5">
        <div class="container-fluid">
		 ...
		 ?>, Please Pay an Amount of Rs <?php echo (isset($_GET['uid']))?$grand:$qty*$price; ?>. And upload the proof below.</p>
                    <form action="<?php echo htmlspecialchars(basename($_SERVER['REQUEST_URI'])); ?>" method="post" enctype="multipart/form-data">

                        <div class="form-group">
                                <label>Item:  <?php // echo prav_get("item_name","item","item_id",$instrument_id);?> </label>
                                <input type="Hidden" placeholder="Item" required name="item_id" class="form-control" value="<?php echo $instrument_id; ?>">
...
?>
						...
						<div class="form-group">
                                <label>Amount</label>
                                <input type="number" placeholder="Price" readonly name="price" class="form-control" value="<?php echo (isset($_GET['uid']))?$grand:$price; ?>">
						...
						<div class="form-group">
                            ...
                            <div class="form-group">
                             ...
                             <img class="i" src="ss1.jpeg" height="200" width="200"></img>
                            </div>
